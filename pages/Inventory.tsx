...
export const Inventory: React.FC = () => {
  ...
  const handleAutoSource = async () => {
    ...
    const newProduct: ProductPack = {
      ...
      qualitySignals: {
        aiReadinessScore: Math.floor(Math.random() * 20) + 80,
        sellerTrust: 85
      },
      attributes: {
  ...
  const aiCount = products.filter(p => p.sourcingType === 'AI').length;
  const aiRate = products.length > 0 ? ((aiCount / products.length) * 100).toFixed(0) : '0';
    ...
    <div className="min-h-screen bg-terminal-bg text-terminal-text p-6">
      ...
      <header className="mb-8 border-b border-gray-800 pb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={isSourcing}
            className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded text-black transition-colors ${isSourcing ? 'bg-purple-900 text-gray-400 cursor-wait' : 'bg-purple-500 hover:bg-purple-400'
              }`}
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="p-4 bg-gray-900/50 rounded border border-gray-800">
          ...
          <div className="text-2xl font-bold text-white">{loading ? '...' : products.length}</div>
        </div>
        <div className="p-4 bg-gray-900/50 rounded border border-gray-800">
          <div className="text-xs text-gray-500 uppercase mb-1">AI Automation Rate</div>
          <div className="text-2xl font-bold text-purple-400">{loading ? '...' : `${aiRate}%`}</div>
        </div>
      </div>
      ...
      <div className="bg-black/30 rounded border border-gray-800 overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-gray-500">Loading products from Supabase...</div>
        ) : (
